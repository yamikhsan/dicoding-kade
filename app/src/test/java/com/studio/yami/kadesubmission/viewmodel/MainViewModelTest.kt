...
class MainViewModelTest {
    ...
    private val model = MainViewModel(repo)
    ...
    fun eventNextLeague() {
        ...
        Mockito.`when`(repo.eventsNextLeague("")).thenReturn(fakeLive)
        model.eventNextLeague("").observeForever(observer)
    ...
    fun eventPastLeague() {
        ...
        Mockito.`when`(repo.eventsPastLeague("")).thenReturn(fakeLive)
        model.eventPastLeague("").observeForever(observer)
        Mockito.verify(observer).onChanged(fakeList)
    }
    ...
    fun lookUpLeague() {
        ...
        Mockito.`when`(repo.lookUpLeague("")).thenReturn(fakeLive)
        model.lookUpLeague("").observeForever(observer)
        Mockito.verify(observer).onChanged(fakeList)
    }
    ...
    fun lookUpEvent() {
        ...
        Mockito.`when`(repo.lookUpEvent("")).thenReturn(fakeLive)
        model.lookUpEvent("").observeForever(observer)
        Mockito.verify(observer).onChanged(fakeList)
    }
    ...
    fun searchEvents() {
        ...
        Mockito.`when`(repo.searchEvents("")).thenReturn(fakeLive)
        model.searchEvents("").observeForever(observer)
